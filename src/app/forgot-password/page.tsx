...
import { Mail, Dumbbell, AlertTriangle, CheckCircle, ArrowLeft } from "lucide-react";
...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (res.success) {
        setMessage(res.message || "A recovery email has been sent if registered.");
      } else {
      ...
      <div className="w-full max-w-md z-10">
        <div className="flex flex-col items-center mb-8">
          ...
          <h2 className="text-3xl font-black uppercase tracking-wider text-white text-center">
            Recover <span className="text-spartan-gold">Your Password</span>
          </h2>
        ...
        <div className="glass-panel-red rounded-lg p-8 shadow-[0_0_50px_rgba(0,0,0,0.8)] border border-white/5 relative">
          ...
          {message && (
            <div className="mb-6 p-4 rounded bg-green-950/20 border border-green-500/50 flex items-start gap-3">
              ...
              <span className="text-sm text-green-200 font-semibold">{message}</span>
            </div>
          )}
